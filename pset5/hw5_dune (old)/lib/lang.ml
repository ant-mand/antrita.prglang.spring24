...
let tm_ascribe (t : term) (ty : typ) : term = TmAscribe(t, ty)
...
let rec substitute x v t = 
  ...
  | _ -> t  (* Extend to handle other cases *)
...
and cbv (t : term) : term option =
  ...
  | TmApp(t1, t2) -> 
    begin match cbv t1 with
    | Some (TmAbs(x, _, body)) ->
        begin match cbv t2 with
...
let extend (g : ctx) (x : string) (t : typ): ctx =
  let new_ctx = List.filter(fun(var, _) -> var <> x) g in (x, t) :: new_ctx
...
let type_infer (g : ctx) (t : term) : typ option = raise Not_implemented

(* return true if t has the given type in the context, else return false *)
let type_check (g : ctx) (t : term) (tp : typ) : bool = raise Not_implemented
